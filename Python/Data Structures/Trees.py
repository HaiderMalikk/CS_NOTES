...
2. **Binary Search Tree (BST)**:
  ...
  - Operations: 
    ...
    - Treversal: To traverse a BST its not linear, we must visit each node once and in a specific order not linear (i.e first second third etc). There are different types of traversal for BST, different alogrithims visit nodes in different orders. here are some:
      - 1. Preorder Traversal: (Given a Non Empty BST) Visit the root node, then the left subtree, then the right subtree. DO THIS RECURSIVELY! meaning the node on left and right subtree will become the new root node and we will do steps 2 and 3 on both left and right nodes over and over Recursilvely until we run out of nodes meaning the node is a leaf node. NOTE: we dont do step one as we alredy checked the left and right nodes of the root node so when the child nodes become the root node they would have alredy been treversed. EX: in the first ex tree after first run we will get in this order: (Root, Child1, Child2) now new root is child 1 once we run out of nodes on left side (child1) we move on to our second root node Child 2 (right subtree)
      - 2. Inorder Traversal: (Given a Non Empty BST) Traverse the left subtree (left most node left of root), then the root node, then the right subtree (right most node node right of root). DO THIS RECURSIVLY!  EX: in the first ex tree after first run we will get in this order: (GG1, G1, null) since G1 has no right node we are done step one NEW root is child one. NOTE!: by doing this traversal we will get the values in order smallest to largest
